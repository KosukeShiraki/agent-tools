...
**「同じ repo には 1 本」と「同時実行は 3 本」は、MCP サーバ 1 プロセスの中でだけ
成り立つ。** 予約も枠もプロセス内の変数なので、別の Claude Code セッションが別の
サーバプロセスを持つと、互いの予約は見えない。

```
セッション A → MCP サーバ A → repo 予約 A / 枠 A
セッション B → MCP サーバ B → repo 予約 B / 枠 B

同じ作業ツリーへの apply を、A と B がそれぞれ「未使用」と判断する
```

同じ `RUNS_ROOT` を使っていても、他サーバの予約を確かめる処理は無い。**端末全体の
保証として扱わないこと。** 別の作業ツリーを使うなら、あるいはサーバが 1 本なら問題は
起きない。

プロセス間ロックにしていないのは、`config.json` のような一瞬の書き換えと違い、run は
数十分走るためである。その間ロックを握り続けると、持ち主が落ちたときに「生きている
ように見える古いロック」を誰が消すかという問題が残り、消し方を誤ると 2 本走る。
**同じ作業ツリーへ書くサーバを 1 本に限る運用**で足りるうちは、そちらを採る。

記録（`runs/`）と設定（`config.json`）は逆で、複数サーバが共有する前提で作ってある
（[設定のロック](#どこで変えるか)、[記録の保護](#設計上の注意)）。`prune` は他サーバが
順番待ちさせている run を消さないし、生死を確認できない run も残す。
...
- **「親 CLI が終了した」と「run が止まった」は別物**: 親が SIGTERM で素直に落ちても、
  SIGTERM を無視する孫が同じプロセスグループに残ることがある。そこで打ち切り中は
  `finalize` を**確定（応答・meta・待ち手の解決）までに留め**、run を手放すのは SIGKILL を
  撃ち終えてからにする。手放すまでは `pendingKills` で追跡を続け、**repo 予約と実行枠を
  握ったまま**にし、`shutdownAll` の停止対象にも残す。親の終了で手放していたときは、
  (1) 次の `apply` が孫の触っている作業ツリーに重なり、(2) その間にサーバが終了すると
  SIGKILL の予約ごと消えて孫だけが残った。(2) は `meta` が既に `killed` なので次回起動の
  孤児回収も拾わない（回収が見るのは `state=running` だけ）——つまり残り続ける。
- **予約した SIGKILL**: `killRun` が仕掛けるタイマーは、run の状態ではなく
  **打ち切りが完了したか**（`record.killing`）で発火を決める。状態で見ると、親の exit で
  `finalize` が走った時点（1 秒後）に撃たれなくなる。完了後に撃つと PID が再利用された
  別のプロセスグループを巻き込むが、SIGTERM から 3 秒以内に PID が一周することは無い。
...
- **起動時刻はキャッシュする**: `currentBootId()` は Windows で PowerShell を起こすため
  ...
  **自分の起動時刻（`currentProcessStartTime()`）も同じ**。run を起こすたびと設定の
  ロックを取るたびに meta へ書くので、引き直していると Windows では毎回 PowerShell が
  起きる（実測 440ms）。**他の pid の起動時刻はキャッシュしてはいけない**——生死と
  PID 再利用の判定に使うため、常に今の値が要る。自分用だけを分けてある。
