#!/usr/bin/env node
// Codex CLI (`codex exec`) を MCP の tool として公開する stdio サーバ。
//
...
// そこに届かないところで切る。長く待ちたい場合は切り離して codex_result を使う。
const MAX_TIMEOUT_MS = 1_500_000; // 25分
const PROGRESS_INTERVAL_MS = 15_000;

// 同時に走らせる codex の本数。detach 後もスロットは保持する（課金が続くため）。
const MAX_CONCURRENCY = Math.max(
  1,
  Number.parseInt(process.env.CODEX_MCP_MAX_CONCURRENCY ?? "", 10) || 3,
);
...
function envDefault(value, fallback) {
...
const TOOL_MODES = {
  codex_consult: {
    ...
    defaultModel: envDefault(
      process.env.CODEX_MCP_CONSULT_MODEL,
      "gpt-6-astra",
    ),
    defaultEffort: envDefault(process.env.CODEX_MCP_CONSULT_EFFORT, "xhigh"),
  },
  codex_apply: {
...
function checkDefaults() {
  ...
  for (const [toolName, mode] of Object.entries(TOOL_MODES)) {
    ...
    if (
      ...
      process.stderr.write(
        `warning: ${toolName} の既定モデル ${mode.defaultModel} は models_cache.json に見当たりません` +
  ...
  return {
    ...
    timeout_ms: {
      ...
      description:
        ...
        "run_id とそこまでの報告を返す。続きは codex_result で取得する。",
    },
    resume_session_id: {
      type: "string",
      description:
...
function toolDefinitions() {
  ...
  return [
    ...
    },
    {
      name: "codex_status",
      description:
        "codex_consult / codex_apply が切り離した run の進捗を見る。" +
      ...
      inputSchema: {
        ...
        properties: {
          run_id: {
            type: "string",
            description: "codex_consult / codex_apply が返した run_id。",
          },
          event_limit: {
            type: "integer",
            minimum: 1,
...
function validateRunId(value) {
  if (!isValidRunId(value)) {
    throw new InvalidArguments(
      `run_id の形式が不正です: ${JSON.stringify(value)}。codex_runs で一覧できます。`,
    );
  }
  return value;
}
...
// 同じ repo で codex_apply を重ねると、互いのファイルを奪い合ううえ、git 差分も
// 混ざって「どの run が何を変えたか」が分からなくなる。repo 単位で 1 本に制限する。
const activeRepos = new Map(); // git root -> run_id

// 完了直後に走らせると、応答を組み立てる前に記録を消しうる。
const PRUNE_DELAY_MS = Math.max(
  0,
  Number.parseInt(process.env.CODEX_MCP_PRUNE_DELAY_MS ?? "", 10) || 10_000,
);
...
async function handleRun(toolName, params) {
    ...
    if (repoRoot === null) {
      return errorResult(
        `cwd が git 管理下にありません: ${cwd}\n` +
          "codex_apply は変更を戻せるように git リポジトリ内でのみ実行できます。" +
          "調査だけなら codex_consult を使ってください。",
      );
    }
    const busy = activeRepos.get(repoRoot);
    if (busy) {
      return errorResult(
        `同じリポジトリで codex_apply が実行中です（run_id=${busy}、repo=${repoRoot}）。\n` +
          "同時に走らせると互いの変更を奪い合い、git 差分もどちらのものか分からなくなります。\n" +
          `codex_status(run_id="${busy}") で進捗を確認するか、完了を待ってください。` +
          "（読むだけなら codex_consult は並行して使えます）",
      );
    }
  }
  ...
  if (resumeSessionId) {
    ...
    if (!origin) {
      return errorResult(
        `resume_session_id がこのサーバの記録に見つかりません: ${resumeSessionId}\n` +
          "codex_runs で session_id を確認してください（他のツールが作ったセッションは再開できません）。",
      );
    }
    if (origin.cwd !== cwd) {
      return errorResult(
    ...
    if (origin.tool && origin.tool !== toolName) {
      toolSwitchNote = `\n（${origin.tool} のセッションを ${toolName} = ${mode.sandbox} で継続します）`;
    }
  }
  ...
  if (!gotSlot) {
    ...
    return errorResult(
      `同時実行の上限（${MAX_CONCURRENCY}）に達しており、${timeoutMs} ms 待っても空きませんでした。\n` +
        ...
        "codex_status で進捗を確認するか、時間をおいて再試行してください。",
    );
  }
  ...
  if (launched.spawnError) {
    ...
    return errorResult(
      `codex の起動に失敗しました: ${launched.spawnError}\n` +
  ...
  if (outcome === "timeout") {
    ...
    return textResult(
      `${header}\n\n⏳ 同期で待つ上限（${timeoutMs} ms）に達したので切り離しました。codex は実行を続けています。\n` +
        ...
        `記録: ${runDir(runId)}\n` +
        `続きは codex_result(run_id="${runId}") で取得できます。codex_status(run_id="${runId}") で進捗を見られます。\n` +
        (body.text
...
function reconstruct(runId, meta, events = readEvents(runId)) {
  ...
  // meta が running のままでも codex 本体が生きているとは限らない（その逆もある）。
  // events に turn.completed があるなら、それが最も確かな完了の証拠なので優先する
  // （pid が別プロセスに再利用されていると、生死判定だけでは永久に「実行中」になる）。
  const alive = meta.state === "running" && isRunAlive(meta);
  const state =
    meta.state === "running"
      ? completed
        ? "completed(推定)"
...
function handleStatus(params) {
  ...
  if (!meta)
    return errorResult(
      `run が見つかりません: ${runId}（codex_runs で一覧できます）`,
    );
...
async function handleResult(params) {
  ...
  try {
    if (record && waitMs > 0) {
      ...
      for (;;) {
        ...
        if (current.turn_completed || !isRunAlive(current)) break;
  ...
  const body = pickBody(runId, live.messages);
  const header =
    `[codex_result] run_id=${runId} tool=${current.tool} state=${live.state}\n` +
    ...
    if (live.alive) {
      return textResult(
        `${header}\n\n⏳ codex（pid=${current.pid}）はまだ動いています。報告はこれから書かれます。\n` +
          `しばらく後に codex_result(run_id="${runId}") を再実行してください。\n` +
...
function handleRuns(params) {
  ...
  const lines = ids.map((runId) => {
    ...
    // codex_status と食い違わないよう同じ基準で出す（件数が多いので events は読まない）。
    let state = meta.state;
    if (getActive(runId)) state = "running[このサーバが追跡中]";
    else if (meta.state === "running") {
      // codex_status と同じ順序（終端の証跡 → 生存 → 不明）で判定する
      state = meta.turn_completed
        ? "completed(推定)"
        : isRunAlive(meta)
          ? "running（別プロセスで継続中）"
          : "unknown";
...
async function handleToolCall(params) {
  ...
  if (name === "codex_result") return handleResult(params);
