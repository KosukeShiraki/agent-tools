...
describe("ディスクの自動回収", () => {
  ...
  before(() => {
    ws = makeWorkspace("disk");
  });
  after(() => rmSync(ws.root, { recursive: true, force: true }));

  it("時間が経った run は workspace だけ落とし、報告は残す", async () => {
    // codex_verify の workspace は test 成果物で重くなる（実測 1 run 170MB）。
    // 記録は軽いので残し、重い方だけ先に回収する。
    const server = startServer(
      ws.env({
        CODEX_MCP_WORKSPACE_TTL_MS: "1",
        CODEX_MCP_PRUNE_GRACE_MS: "1",
        CODEX_MCP_PRUNE_DELAY_MS: "1",
      }),
    );
    try {
      const first = await server.call(
        "codex_verify",
        { prompt: "テストして", target_dir: ws.gitDir },
        {},
        20_000,
      );
      const runId = runIdOf(textOf(first.result));
      const dir = join(ws.runsDir, runId);
      assert.ok(existsSync(join(dir, "workspace")), "workspace が作られていない");

      // 次の run の完了で prune が走る
      await server.call("codex_verify", { prompt: "次", target_dir: ws.gitDir }, {}, 20_000);
      await new Promise((r) => setTimeout(r, 600));

      assert.ok(!existsSync(join(dir, "workspace")), "workspace が回収されていない");
      // 報告と記録は残す（後から codex_result で読めること）
      assert.ok(existsSync(join(dir, "meta.json")), "meta.json まで消えた");
      assert.ok(existsSync(join(dir, "messages.jsonl")), "報告まで消えた");

      const result = await server.call("codex_result", { run_id: runId });
      assert.equal(result.result.isError, undefined, textOf(result.result));
      assert.match(textOf(result.result), /FAKE_ANSWER|PARTIAL_REPORT/);
    } finally {
      server.close();
    }
  });

  it("共有 cache は上限を超えたら捨てる", async () => {
    const server = startServer(
      ws.env({
        CODEX_MCP_MAX_CACHE_BYTES: "100",
        CODEX_MCP_PRUNE_DELAY_MS: "1",
        CODEX_MCP_CACHE_CHECK_INTERVAL_MS: "1", // 測定間隔の既定は 1 時間
      }),
    );
    try {
      await server.call("codex_verify", { prompt: "x", target_dir: ws.gitDir }, {}, 20_000);
      const cacheDir = join(ws.runsDir, ".cache");
      // 上限を超える中身を置く
      writeFileSync(join(cacheDir, "uv", "big.bin"), Buffer.alloc(4096));
      assert.ok(existsSync(join(cacheDir, "uv", "big.bin")));

      await server.call("codex_verify", { prompt: "y", target_dir: ws.gitDir }, {}, 20_000);
      await new Promise((r) => setTimeout(r, 600));
      // 丸ごと捨てられるか、次の run が作り直した空の状態になっている
      assert.ok(!existsSync(join(cacheDir, "uv", "big.bin")), "cache が回収されていない");
    } finally {
      server.close();
    }
  });
